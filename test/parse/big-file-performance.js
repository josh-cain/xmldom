...
describe('XML Node Parse', () => {
  it("big file parse", () => {
    ...
    var domjsTime = t3-t2;
    expect(domjsTime).to.be.above(xmldomTime,'xmldom performance must more height!!');
    ...
    const xmldomresult = (domjs(doc1+'')+'').replace(/^<\?.*?\?>\s*|<!\[CDATA\[([\s\S]*?)\]\]>/g,xmlReplace)
    const domjsresult = (doc2+'').replace(/^<\?.*?\?>\s*|<!\[CDATA\[([\s\S]*?)\]\]>/g,xmlReplace)
    data = xmldomresult;
    ...
    if(xmldomresult !== domjsresult){
      for(var i=0;i<xmldomresult.length;i++){
        if(xmldomresult.charAt(i)!=domjsresult.charAt(i)){
          ...
          console.log('pos'+i,'\n',xmldomresult,'\n\n\n\n',domjsresult)
          expect(xmldomresult).to.equal(domjsresult);
          break;
        }
      } 
